...
const DogTable = ({ items, onSelect, classes, t, Actions }) => {
  ...
  return items.length ? (
    ...
    <div className={classes.noItems}>{t('entities.dog.noItems')}</div>
  );
};

DogTable.propTypes = {
  items: PropTypes.arrayOf(dogType).isRequired,
...
DogTable.defaultProps = {
  onSelect: () => {},
  Actions: null,
};

export default withStyles(styles)(withTranslation()(DogTable));
