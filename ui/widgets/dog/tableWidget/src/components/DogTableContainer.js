...
import { apiDogsGet, apiDogsDelete } from 'api/dogs';
import { reducer, initialState } from 'state/dog.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/dog.types';
...
class DogTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { dogs, headers } = await apiDogsGet(serviceUrl, requestParameters);
        const dogCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...dogs] : dogs,
            count: dogCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiDogsDelete(serviceUrl, item.id);
        onDelete(item);
    ...
    const Actions = ({ item }) =>
      onDelete ? (
        <ConfirmationDialogTrigger
          ...
          dialog={{
            title: t('entities.dog.deleteDialog.title'),
            description: t('entities.dog.deleteDialog.description'),
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <DogTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
DogTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(DogTableContainer))
  )
);
