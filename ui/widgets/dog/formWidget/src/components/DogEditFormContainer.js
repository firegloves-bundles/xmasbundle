...
import { apiDogGet, apiDogDelete, apiDogPut } from 'api/dogs';
import Notification from 'components/common/Notification';
import DogForm from 'components/DogForm';

class DogEditFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleSubmit(dog) {
    ...
    if (authenticated) {
      try {
        const updatedDog = await apiDogPut(serviceUrl, dog);
        onUpdate(updatedDog);
  ...
  async handleDelete(dog) {
    const { t, onDelete, keycloak, serviceUrl } = this.props;
    const authenticated = keycloak.initialized && keycloak.authenticated;

    if (authenticated) {
      try {
        await apiDogDelete(serviceUrl, dog.id);
        onDelete(dog);
        this.setState({
          dog: null,
  ...
  render() {
    ...
    const { notificationMessage, notificationStatus, dog } = this.state;

    let form;
    if (typeof dog === 'undefined') {
      form = t('entities.dog.notFound');
    } else if (dog === null) {
      form = t('entities.dog.deleted');
    } else {
      form = (
        <DogForm
          dog={dog}
...
DogEditFormContainer.propTypes = {
...
DogEditFormContainer.defaultProps = {
  onCancelEditing: () => {},
  onDelete: () => {},
  onUpdate: () => {},
  onError: () => {},
...
export default withKeycloak(withTranslation()(DogEditFormContainer));
